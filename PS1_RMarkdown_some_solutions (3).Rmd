...
```{r ndist}
tmp = ggplot(DJ_d, aes(sample=(r_Dow_Jones-mean(r_Dow_Jones))/sd(r_Dow_Jones)))
norm_plot1 <- tmp + geom_qq(distribution=stats::qnorm, dparams=list(mean=0, sd=1)) + geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Daily Log Returns")

tmp = ggplot(DJ_w, aes(sample=(r_close-mean(r_close))/sd(r_close)))
norm_plot2 <- tmp + geom_qq(distribution=stats::qnorm, dparams=list(mean=0, sd=1)) + geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Weekly Log Returns")

norm_plot1/norm_plot2
```

The first thing we observe when studying above plots are that the line intersects the data in the middle quite good. When it comes to the tails, we observe that the normal distribution captures these observations badly. This would indicate on a heavier-tailed data than the normal distribution would show. This finding is in accordance with the stylized fact about asset returns that the empirical distribution of asset returns show fat tails compared to the normal distribution.

#### b) alternative tn-distributions with n degrees of freedom.

Since we know t distribution has fatter tails, we test against the location-scale t distribution $lst(\mu,\tau^2,\nu)$, with same mean $\mu$ and same variance $\tau^2 \frac{\nu}{\nu-2}$ (for $\nu>2$). Since location-scale t distribution is not in base R, we use extraDistr:qlst" to produce the qq-plots. We try with $\nu=5,4,3$.

 
```{r tdist, message=FALSE, include=FALSE}
library(extraDistr)
df=5
tmp = ggplot(DJ_d, aes(sample=(r_Dow_Jones)))
t_plot_1_df5<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_d$r_Dow_Jones),
                                                                        sigma=sqrt(var(DJ_d$r_Dow_Jones)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Daily Log Returns, df=5")

tmp = ggplot(DJ_w, aes(sample=(r_close)))
t_plot_2_df5<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_w$r_close),
                                                                        sigma=sqrt(var(DJ_w$r_close)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Weekly Log Returns, df=5")

df=4
tmp = ggplot(DJ_d, aes(sample=(r_Dow_Jones)))
t_plot_1_df4<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_d$r_Dow_Jones),
                                                                        sigma=sqrt(var(DJ_d$r_Dow_Jones)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Daily Log Returns, df=4")

tmp = ggplot(DJ_w, aes(sample=(r_close)))
t_plot_2_df4<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_w$r_close),
                                                                        sigma=sqrt(var(DJ_w$r_close)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Weekly Log Returns,df=4")


df=3
tmp = ggplot(DJ_d, aes(sample=(r_Dow_Jones)))
t_plot_1_df3<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_d$r_Dow_Jones),
                                                                        sigma=sqrt(var(DJ_d$r_Dow_Jones)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Daily Log Returns, df=3")

tmp = ggplot(DJ_w, aes(sample=(r_close)))
t_plot_2_df3<- tmp + geom_qq(distribution=extraDistr::qlst, dparams=list(df=df,
                                                                        mu=mean(DJ_w$r_close),
                                                                        sigma=sqrt(var(DJ_w$r_close)*(df-2)/df))) +
  geom_abline(aes(intercept=0,slope=1),color="red")+ggtitle("Standardized Weekly Log Returns, df=3")
```
