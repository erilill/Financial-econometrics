...
summary(lret)
```
Above we generate the log returns, `lret`. Here we also see some summary statistics. Below the log returns are plotted:

```{r}
matplot(lret,type='l',ylab='log returns',xlab='time horizon')
```
Here we see that there are a few larger spikes showing periods of higher volatility, but generally the plot shows low volatility with smaller spikes for most time periods.
```{r}
sum(is.na(lret[,'FRCAC40']))
```
We see that we have missing values in the series `FRCAC40`, 393 missing values.
