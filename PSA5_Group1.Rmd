...
For the AIC, it suggests that $GARCH(2,1)$ is slightly slightly better, with $GARCH(1,1)$ at the second place. Although BIC suggests $GARCH(1,1)$ is the best model. This suggests that the 1,1 order for the $GARCH(1,1)$ is good enough, and if BIC decides it is actually the optimal lag order.
...
Now we reestimate the model based on a t-distribution and re-evaluate the distributional properties of the resulting standardized residuals.
...
In this comparison we would expect that Gaussian GARCH produces more conservative values, however, they do not. The estimates from Gaussian GARCH are significantly less negative than from the model with the unconditional mean. This could suggest that the markets experienced quite stable volatility since ??? 
