...
quantiles <- qnorm(c(0.1, 0.05, 0.01, 0.005, 0.001))

V_t_d <- 1

VaR_normal_d <- V_t_d * (exp(quantiles * sqrt(sigma2) + mu) - 1)

VaR_normal_d

V=10
h <- c(1,10,30)
phi_inv_p <- qnorm(0.01)
mu=0.001
sigma2=0.015^2
round(V*(exp(phi_inv_p*sqrt(h*sigma2)+h*mu)-1),3)
...
mu=mean(r_d)
sigma2=var(r_d)
round(V*(exp(qnorm(p)*sqrt(h*sigma2)+h*mu)-1),3)

VaR_normal <- function(r,p,h,V){
  mu = mean(r)
  sigma2 = var(r)
  phi_inv_p <- qnorm(p)
  V*(exp(phi_inv_p*sqrt(h*sigma2)+h*mu)-1)
}

p <- c(0.1, 0.05, 0.01, 0.005, 0.001)
h <- c(1,10,30)

VaR_normal(r_d,p,h,1)
