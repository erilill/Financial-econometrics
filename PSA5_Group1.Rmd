...
Pacf(r^2, lag.max=12)
```
We can see that there is not much persistance in the log returns, but there is persistance in the squared log returns. To specify the mean equation we use auto.arima again
...
Now we look at the ACF plots and do box tests for the mean corrected returns and squared mean corrected returns
...
VaR_df <- data.frame(
  ...
  mu = rep(mean(r_w), length(p)), 
  sigma2 = rep(var(r_w), length(p)),
...
kable(
  VaR_df,
  col.names = c("Probability", "mu", "sigma2", "Horizon (h=1)", "Horizon (h=10)", "Horizon (h=30)", "V"),
  caption = "Value-at-Risk Estimates from unconditional normal DJ Daily",
  digits = 3
)
```
...
kable(
  VaR_df,
  col.names = c("Probability", "mu", "sigma2", "Horizon (h=1)", "Horizon (h=4)", "Horizon (h=8)", "V"),
  caption = "Value-at-Risk Estimates from unconditional normal on DJ Weekly",
  digits = 3
)
```
...
kable(
  VaR_df,
  col.names = c("Probability", "mu", "sigma2", "Horizon (h=1)", "Horizon (h=10)", "Horizon (h=30)", "V", "nu"),
  caption = "Value-at-Risk Estimates from unconditional normal DJ Daily",
  digits = 3
)
```
...
V <- 1
h <- c(1,10,30)
VaR_ARMA_GARCH(m12,p,h,V) #daily
```
...
```{r}
h <- c(1,4,8)
VaR_ARMA_GARCH(m13,p,h,V) #weekly
```
...
V <- 1
h <- c(1,10,30)
m14_daily <- VaR_ARMA_GARCH(m14,p,h,V) #daily

VaR_table <- data.frame(matrix(unlist(m14_daily), ncol = length(h), byrow = TRUE))
colnames(VaR_table) <- paste("Horizon", h)
rownames(VaR_table) <- paste("p =", p)
kable(VaR_table, caption = "Value-at-Risk for daily data of a position V = 1 for different horizons using a GARCH(1,1) model based on the t-distribution with constant conditional mean function.")

h <- c(1,4,8)
m15_weekly <- VaR_ARMA_GARCH(m15,p,h,V) #weekly
VaR_table <- data.frame(matrix(unlist(m15_weekly), ncol = length(h), byrow = TRUE))
colnames(VaR_table) <- paste("Horizon", h)
rownames(VaR_table) <- paste("p =", p)
kable(VaR_table, caption = "Value-at-Risk for weekly data of a position V = 1 for different horizons using a GARCH(1,1) model based on the t-distribution with constant conditional mean function.")
```
