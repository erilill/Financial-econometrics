...
```
We can see that the residuals are serially correlated, since we reject the null of the first 20 autocorrelations being zero. So the ARMA mean function does not seem to properly remove the linear dependence in the data. Anyways we proceed.
...
```
We can see that the ACF/PACF plots suggest litte to no autocorrelation in the mean-corrected returns. The Ljung-Box test furthermore does not reject the null of no autocorrelation in the mean-corrected returns.
