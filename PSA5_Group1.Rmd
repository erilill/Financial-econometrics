...
VaR_normal_ARMA_GARCH <- function(model,p,h,V){
  ...
  pred <- fGarch::predict(model,n.ahead=h)
  mupred <- pred$meanForecast
  sigma2pred <- mpred$standardDeviation^2
...
m12pred <- fGarch::predict(m12,n.ahead=30,p_loss=c(0.1))
mupred <- m12pred$meanForecast
sigma2pred <- m12pred$standardDeviation^2

VaR_normal_ARMA_GARCH(mupred,sigma2pred,p,h,V)
```
