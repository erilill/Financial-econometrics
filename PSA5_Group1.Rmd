...
```{r, echo=F}
test <- garchFit(r_d ~ garch(1, 1), cond.dist ="norm", data = r_d, trace = F)
...
for (i in 1:length(probs)){
  tmp[,i] <-VaR(test, p_loss=probs[i])
}


matplot(tmp, type = "l", lty = 1, col = 1:ncol(tmp), xlab = "Time", ylab = "VaR", main = "VaR over time")
...
Comparing daily normal with daily t 
Comparing daily normal with daily t-distribution we would expect more conservative estimates(i.e. more negative estimated values) and we also observe that the t-distribution produces more conservative estimates. All of the curves are steeper. 

Comparing weekly normal with weekly t-distribution
Once again we observe more conservative estimates for a t-distribution. This finding in addition to the previous one would indicate that the data exhibits some heavy-tails. 

Comparing daily normal with weekly normal 
When comparing the daily and weekly data using the same model we would expect weekly data to be more conservative. This would be according to theory that weekly data compounds risk over larger horizons. Not even that, the slopes are steeper in weekly data. 

Comparing daily t with weekly t 
Also when comparing the different data for t-distribution we observe once again that weekly data exhibits more conservative estimates. 

Comparing daily Gaussian GARCH with daily t-GARCH
These plots would be almost identical. Here we would expect to have once again more conservative estimates for t-GARCH, however, the empirical findings in this case doesn't support this theory particularly strong. 

Comparing weekly Gaussian GARCH with weekly t-GARCH 
Once again the expectation is to have more conservative estimates for t-GARCH even for weekly data, however, we cannot find support for this theory as the Gaussian GARCH seems to produce the most conservative output. 

Comparing daily Gaussian GARCH with weekly Gaussian GARCH
We expected weekly Gaussian GARCH to provide more negatives estimates as the risk is compounded over longer periods. This is found when comparing the two different plots. IN addition, we also observe that daily data seems to diverge more than weekly for longer horizons. 

Comparing daily t-GARCH with weekly t-GARCH
These findings are very close to the previous ones for daily Gaussian GARCH and weekly Gaussian GARCH. Weekly t-GARCH seem to have more conservative estimates but daily data seems to diverge more than weekly in longer horizons. 

Comparing daily normal with daily Gaussian GARCH
In this comparison we would expect that Gaussian GARCH produces more conservative values, however, they do not. The estimates from Gaussian GARCH are significantly less negative than from the model with the unconditional mean. 

Comparing daily t with daily t-GARCH
Once again we observe that the daily t produces much more conservative estimates than the t-GARCH. 

Comparing weekly normal with weekly Gaussian GARCH
Once again we observe that the weekly normal produces more conservative estimates than the Gaussian GARCH. 

Comparing weekly t with weekly t-GARCH
Lastly, we observe the same finding that the t-GARCH produces less conservatives estimates compared to the unconditional model. 

In conclusion we find three findings in this task: 
t-based models produce in general more conservative estimates, capturing better the presence of heavy tails in the returns. 
GARCH models seem to yield less conservative estimates compared to unconditional mean-based models. 
Lastly we observed that weekly data exhibits more conservative estimates than daily data.
