...
Firstly, in question 1 we could see some evidence pointing towards predictability in daily data but not in weekly data. The ACF plots had a couple of significant spikes for daily data and barely any for weekly data. This findings are confirmed by Ljung-Box test which were significant for daily data but not for weekly data. Also the VR/VD was significant for daily but not for weekly. 

Secondly, we analyzed the aggregated returns which were very similar to the findings in question 1. We saw some evidence in the ACF plots for 2d daily data but not for weekly data. The Ljung-Box test confirmed these findings, being significant for 2d aggregated data but not for 2w aggregated data. In the VR/VD test, the test statistics were significant for all periods except for when q = 2. For weekly not one test was significant. 

Thirdly, in question 3 we checked some for different periods and saw some weak evidence of structural breaks. In daily data, we saw that period 3 were slightly different since the first two periods had more significant lags up to lag 5. For weekly data there was no indication that the different periods were different. For aggregated returns we saw that period 1 was slightly different in the most recent lags compared to the two successive periods and for weekly data we saw some weak evidence that period 2 were different from the other periods. 

Given all these findings, we can conclude that asset predictability is possible for daily data, even if it is aggregated but not for weekly data under any circumstances. This is since for daily data, all tests and plots we do points towards asset predictability. For weekly data it is almost the opposite, there are barely any evidence pointing towards asset predictability and on the contrary pointing towards that the series is a random walk. Given the findings in the ACF plots, this holds mostly for recent lags. There was one finding that was a slightly contradictory to this, for 2-day aggregated return the insignificant VR/VD test for q = 2 but this could also be due to randomness since all other tests pointed towards asset predictability, especially in the recent lags. 
...
We can see that we reject the null of the first 6 and 12 autocorrelations being zero for both low 10\% and high 10\%, which indicates market predictability for both. Although we reject much more strongly for low 10 \%, which indicates what market predictability is more prevelant for lower capitalized stocks than high.
...
\end{tabular}
\end{table}
In table \ref{tab:VDRB22} we see the results from the variance ratio test for the two month aggregated portfolio returns. Here we see the results that we cannot reject the null and come to the conclusion that the series are not 
...
When negative lag, it means that the returns of Lo_10 is compared to the future returns of the Hi_10. When positive lags it means that the Hi_10 is compared to the future returns of the Lo_10. We observe significant lags both for negative and positive lags. Furthermore, the lags are in both directions, both positive and negative cross autocorrelations and for this reason it is very hard to draw a conclusion one general conclusion. One thing that is evident is however the significant lag 1, which means that Hi_10 is correlated with future returns of Lo_10, supporting the theory that big companies leads and small companies lags. We will continue by dividing it into subperiods and examining the autocorrelations again. Above is a table of the magnitudes. 
```{r}
...
```
In this case, all periods except for the third are significant at a 5\% level, the third is significant at 10\% level. This concludes and proves the theory that large companies are leading. 
...
Yes, we can confirm the results of Campbell/Lo/Mackinlay (1997) and the theory that high-cap stocks lead small-cap stocks. The results are stable over time on a 10\% level and on a 5\% level on the whole dataset and in period 1 and 3. 
