...
Even after dividing into sub-periods, it is the same story as before. The ACF plot tell us that in all periods there is quite strong evidence for predictability for the daily data, and not so much for the weekly data.
```{r}
...
```
The Box-Ljung test although now paint a different picture. At least for daily, it is like before, we reject the nulls and there is strong evidence for market predictability in all periods. But for weekly, we now see that on a 10% level we now reject the null in all periods, so for each period, we see evidence for predictability for weekly data, which we did not see before.
...
Given all these findings, we can conclude that asset predictability is probable for the daily data, even if it is aggregated but not for weekly data. This is since for daily data, all tests and plots we do points towards asset predictability. For the weekly data we see no evidence pointing towards asset predictability and on the contrary, we see indicatoins that the series is a random walk. Given the findings in the ACF plots, this holds mostly for recent lags. There was one finding that was a slightly contradictory to this, for 2-day aggregated return the insignificant VR/VD test for q = 2 but this could also be due to randomness since all other tests pointed towards asset predictability, especially in the recent lags. 
...
\end{tabular}
\end{table}
What we see in table \ref{tab:LB_test} is that the auto-correlations for lags 6 and 12 are significant at $\alpha=0.05$ for the two month aggregated portfolio returns for low 10\%, but for top 10\% the auto-correlations are not significant. This indicates that for two month aggregated returns, asset return predictability only holds for the low 10\%.
