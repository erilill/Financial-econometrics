...
>>>>>>> f9f4dc42e26b9bc7ecf9d0dbac0953d27d133895
...
lo20 <- portcap_m$Lo_20 / 100 
hi20 <- portcap_m$Hi_20 / 100
lo30 <- portcap_m$Lo_30 / 100 
hi30 <- portcap_m$Hi_30 / 100
```
...
for (i in 1:length(q)) {
  res_1[[i]] <- c(VDR(lo10, iq = q[i]), list(q = q[i]))  # Include q in the result
  res_2[[i]] <- c(VDR(hi10, iq = q[i]), list(q = q[i])) # Include q in the result
}
...
par(mfrow=c(1,1))
```
Looking at the ACF-plots above we see significant auto-correlations for both the low and top 10\%. Positive auto-correlations indicates that there could
>>>>>>> 4aa4528220fd6d127a8af8677faf313d37b214d5
