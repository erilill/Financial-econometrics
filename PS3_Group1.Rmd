...
>>>>>>> 8974e619ff78e30459e9a6d270f2df2435884a80
...
According to theory CAPM predicts that the intercept $\gamma_{0t}$ is zero. If it is not equal to zero then the test implies that some returns are not explained by the market beta and thus there is model misspecification. In period 1 the p-value is 0.7921 so we fail to reject the null hypothesis that $\gamma_{0t}=0$ for all t. For period 2 the p-value is 0.1616 and once again we fail to reject the null. For the last period the obtained p-value is less than 0.05 and therefore we reject the null that $\gamma_{0t}=0$. Rejection of $\gamma_{0t}=0$ suggests that the second pass regression indicates unexplained returns. This means that for this period, the CAPM does not fully explain the returns for this period. This can occur if there are some structural breaks in the data, market anomalies or some macroeconomic events which affect returns. 

### (b) Test whether \(\gamma_{1t}>0\) for all t

Now we actually test the null in the other direction $\gamma_{1t}\le0$ because then we get the alternative that is strictly larger, i.e. $\gamma_{1t}>0$. Here we conduct a one-sided t-test. 
...
```
<<<<<<< HEAD
Theory says we should reject $\gamma_{1t}$ is zero. We can see that for period 1, we reject the null of $\gamma_{1t}\le0$ for all t at a 5% significance level, so we accept the alternative of $\gamma_{1t}>0$. For period 2 and 3, we accept the null that the market risk premia is less than or equal to zero $\gamma_{1t}\le0$ actually. This finding in period 2 and 3 of the estimated market risk premia contradicts the theory that investors holding systematic risk should be awarded higher.  
=======
Theory says we should reject $\gamma_{1t}$ is zero or less, because we believe it to be positive. We can see that for period 1, we reject the null of $\gamma_{1t}\le0$ for all t at a 5% significance level, so we accept the alternative of $\gamma_{1t}>0$. For period 2 and 3, we accept the null that the market risk premia is less than or equal to zero $\gamma_{1t}\le0$ actually. This contradicts the theory that investors holding systematic risk are not rewarded. 
>>>>>>> 8974e619ff78e30459e9a6d270f2df2435884a80
