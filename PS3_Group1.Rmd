...
# Det här är rätt bajs, ni får jättegärna ändra om ni vill
...
colnames(period_spec) <- c("Period2Coef", "PValuePeriod2", "Period3Coef", "PValuePeriod3", 
                           "InteractionPeriod2Market", "PValueInteractionPeriod2Market", 
                           "InteractionPeriod3Market", "PValueInteractionPeriod3Market")

kable(period_spec, digits = 3, caption = "Estimated CAPM for the portfolios", label = "tab:capm3")
...
```{r}
# Kai rekommenderade Chow test för a)
# Chow test
...
capm_modelR11 <- lm(excess_returnsR11 ~ excess_returnsMarket)

S_C <- sum(capm_modelR11$residuals^2)
...
capm_modelR11_t1 <- lm(excess_returnsR11_t1 ~ excess_returnsMarket_t1)
capm_modelR11_t2 <- lm(excess_returnsR11_t2 ~ excess_returnsMarket_t2)

S_1 <- sum(capm_modelR11_t1$residuals^2)
S_2 <- sum(capm_modelR11_t2$residuals^2)

Chow_test_stat <- ((S_C-(S_1+S_2))/k)/((S_1+S_2)/(N_1+N_2-2*k))


pval = 1-pf(Chow_test_stat,df1=k,df2=N_1+N_2-2*k) #0.802

my_chow.test <- c(Fstat=Chow_test_stat, df1=k,df2=N_1+N_2-2*k, pvalue=pval)
...
my_chow.test
chow.test(y1,x1,y2,x2) #same results
```
The obtained p-value is 0.802, so we reject that $\alpha_1=\alpha_2,\beta_1=\beta_2$, i.e. there seems to be time-varying market betas and intercept terms for the R11 portfolio.

Tänker att vi kör en for loop över alla Rxx portfolios, se hur många pval<0.05.
