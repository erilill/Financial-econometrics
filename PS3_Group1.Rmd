...
for (i in portfolios) {
  ...
  capm_model <- lm(ER ~ excess_returns$Market + portfolio_m$Period2 + 
                     portfolio_m$Period3 + 
                     portfolio_m$Period2 * excess_returns$Market + 
                     portfolio_m$Period3 * excess_returns$Market)
...
We can perform the Chow Test. We estimate the full sample CAPM for portfolio i as
...
r222 <- rbind(num_higher_r2, num_higher_adj_r2)
rownames(r222) <- c("Number of portfolios where R² is higher with period-specific terms:", "Number of portfolios where adjusted R² is higher with period-specific terms:")
kable(r222, caption = "Comparison of models with and without period-specific terms")
```
Looking at the CAPM where we include period-specific effects, we see that all individual $R^2$ are larger compared to the original CAPM. Looking at the adjusted $R^2$, we see that 95/100 $R^2_{adj.}$ are larger. However, this is not necessarily an indication that goodness-of-fit drastically improves. Not included in the report, but looking at the difference between these individual $R^2$, we see that the differences are very small between the two CAPM models. 
...
So we implement the regression
...
Note that we do the assumption that $\gamma_{0t}$ are i.i.d. which is not a realistic assumption but it is a compromise.

The test statistic is
$$
w(\hat{\gamma}_0)=\frac{\hat{\gamma}_0}{\hat{\sigma}_{\gamma_0}} \sim t_{T-1}
$$

where $\hat{\gamma}_0=\frac{1}{T}\sum_{t=1}^T \hat{\gamma}_{0t}$ and $\hat{\sigma}_{\gamma_0}=\frac{1}{T(T-1)}(\hat{\gamma}_{0t}-\hat{\gamma}_{0})^2$.
...
So $\gamma_1$ is the risk premia and according to theory it is positive, i.e. we should reject that it is zero or less. It should be positive, because the investor should get some positive return in excess of the risk-free rate, as a reward for taking on more risk. So we test the null $\gamma_{1t}\le0$ for all t because then we get the alternative that is strictly larger, i.e. $\gamma_{1t}>0$.

Here we conduct a one-sided t-test. Note that we do the  assume that $\gamma_{1t}$ are i.i.d. which is not a realistic assumption but it is a compromise.

The test statistic is
$$
w(\hat{\gamma}_1)=\frac{\hat{\gamma}_1}{\hat{\sigma}_{\gamma_1}} \sim t_{T-1}
$$

where $\hat{\gamma}_1=\frac{1}{T}\sum_{t=1}^T \hat{\gamma}_{1t}$ and $\hat{\sigma}_{\gamma_1}=\frac{1}{T(T-1)}(\hat{\gamma}_{1t}-\hat{\gamma}_{1})^2$.
...
We can see that for period 1, we reject the null of $\gamma_{1t}\le0$ for all t at a 5% significance level, so we accept the alternative of $\gamma_{1t}>0$. So period 1 is according to theory. For period 2 and 3, we accept the null that the risk premia is less than or equal to zero $\gamma_{1t}\le0$ actually. This finding in period 2 and 3 of the risk premia contradicts the theory.
...
```
Note that we get exactly the same results as before, so no need to interpret again. This is because now we calculate the t-statistic another way by

$$
\frac{\frac{1}{T}\sum_{t=1}^T \hat{\gamma}_{0t}-0}{\hat{\sigma}_{\hat{\gamma}_{0}}/\sqrt{T}} \sim t_{T-1}
$$

but it is the same actually.
...
Note that we again get exactly the same results as before, so no need to interpret. This is because now we calculate the t-statistic another way by

$$
\frac{\frac{1}{T}\sum_{t=1}^T \hat{\gamma}_{1t}-0}{\hat{\sigma}_{\hat{\gamma}_{1}}/\sqrt{T}} \sim t_{T-1}
$$
...
```
Goodness-of-fit provide evidence that for period 1 the betas explains around 68\% of the variability in the excess returns. For period 2 we see a higher $R^2$ around 73\% and for period 3 we observe $R^2$ around 75\%. We see very similar results for the adjusted $R^2$ to the standard $R^2$.

We observe that the results from the hypothesis testing fluctuates over time and in some periods contradict the theory. Given the findings that the results fluctuate there seems to be evidence that the theory is not so robust over time. 
