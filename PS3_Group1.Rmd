...
hist(fpreg$R2)
```
We clearly see the left skewness, i.e. a lot of higher $R^2$ happens to the right, so in essence in most cases the $R^2$ is really high/good/, i.e. the CAPM model captures a lot of variation in the data. 
...
$$
Now we want to verify to what extent this is correct. We do not have the expectations, so a (not so mathematically rigorous) compromise is to use the averages, and see if it holds, i.e.
