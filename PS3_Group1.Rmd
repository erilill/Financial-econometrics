...
```
In this question we will use the `FamaMacbeth0()` function which was given in the assignment in order to estimate the joint CAPM and test for joint significance of the intercept terms using the F-test.

The test used is the F-test proposed by Gibbons/Ross/Shanken 1989:
...
```
In table \ref{tab:Ftest} we see the results from the F-test. Here we test if $\alpha=0$, we see in the table that the p-value is very close to zero which means that we reject the null hypothesis, suggesting that the intercept is significantly different from zero.
