...
beta_variation_df <- data.frame(
  ...
  Size = size_decile,
  BookToMarket = book_to_market_decile,
...
Based on this plot, it seems that high book-to-market portfolios are most volatile over time as the variation in beta is high for most portfolios with high book-to-market. There also seem to be a relationship between size and time variation, as those of smaller size seem to have larger variation. In contrast, larger portfolios with low book-to-market are the least volatile.
...
We can see that is quite heavily left skewed. Also bla bla etc.

```{r}
hist(fpreg$R2)
```
We clearly see the left skewness, i.e. a lot of higher $R^2$ happens to the right, so in essence in most cases the $R^2$ is high/, i.e. the CAPM model captures a lot of variation in the data. 
...
for (i in 1:length(subsamples)){
  mR = as.matrix(portfolio_m[subsamples[[i]],25:124])
  Rf = as.matrix(portfolio_m[subsample[[i]],'Tbill'])
  Rm = as.matrix(portfolio_m[subsample[[i]],'Market'])
  mZ = sweep(mR,1,Rf)
  vZm = Rm - Rf
  ...
  pval_wgamma0[[i]] <- pval <- 2*pt(abs(wgamma0), df=iT-1,lower.tail=FALSE)
  wgamma0_list[[i]] <- wgamma0
  # b)
  wgamma1 <- SPreg$wgamma1
  pval_wgamma1 <- pt(wgamma1, df=iT-1, lower.tail=FALSE)
  wgamma1_list[[i]] <- wgamma1
  # c)
  (Tstat <- (mean(gamma0)-0)/(sd(gamma0)/sqrt(length(gamma0))))
  (wgamma0)

  p <- 2*pt(abs(Tstat), df=length(gamma0)-1,lower.tail=FALSE))
}
```
...
```{r}
(wgamma1 <- spreg$wgamma1)
(pval <- pt(wgamma1, df=iT-1, lower.tail=FALSE))
```
Our p-value is $<0.01$ so we reject the null at a 1% significance level that $\gamma_{1t}>0$ for all t.
...
(pval <- 2*pt(abs(Tstat), df=length(gamma0)-1,lower.tail=FALSE))
```
...
(pval <- pt(wgamma1, df=iT-1, lower.tail=FALSE))
```
We assuem the gammas are i.d.d. This is a strong assumption and not realistic. But still, if we assume, we can do it.
