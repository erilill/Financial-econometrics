...
variation_threshold <- quantile(beta_variation, 0.9)
high_variation <- beta_variation > variation_threshold

# Print flagged portfolios
cbind(betas,beta_variation)[high_variation,]
```
In the output above we have printed the 10 portfolios which have the largest variation in betas between the time periods. Here we see that portfolios with higher book-to-market ratio generally have higher variation between the time periods. Size seems to matter less as we have examples from the 1-7th deciles in the output above. There are however some exceptions, namely R21 and R41 which both have lower book-to-market ratio.
...
FirstPass <- function(mZ, vZm){
  ...
  return(list(alpha=alpha,beta=beta,R2=R2,mu=mu))
}
fp_reg <- FirstPass(mZ,vZm)
```
