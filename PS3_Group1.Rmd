...
Now lets plot the histogram of the betas
```{r}
hist(fpreg$beta)
```
It does not look like a normal distribution. Etc.
...
Tstat_list_gamma0_v <- numeric(length(subsamples))
pval_avg_gamma0_v <- numeric(length(subsamples))

Tstat_list_gamma1_v <- numeric(length(subsamples))
...
subsample3 <- 629:942
  ...
  mR = as.matrix(portfolio_m[subsample3,25:124])
  Rf = as.matrix(portfolio_m[subsample3,'Tbill'])
  Rm = as.matrix(portfolio_m[subsample3,'Market'])
  mZ = sweep(mR,1,Rf)
  vZm = Rm - Rf
...
(pval <- pt(Tstat, df=iT-1, lower.tail=FALSE))
```
...
>>>>>>> 71a7e616311882bb0abf47d5dd76e90bcfafcecc
