...
for (i in 25:124){
  ER <- as.matrix(portfolio_m[,i]-portfolio_m$Tbill) #example R11, column 25
  y1 <- ER[t1]
  y2 <- ER[t2]
...
```
The obtained p-value is 0.802, so we do not reject that $\alpha_1=\alpha_2,\beta_1=\beta_2$, i.e. there seems to be no time-varying market betas and intercept terms for the R11 portfolio, as they are not significantly different from each other on a 1% level.

For 80 out of the 100 portfolios, we reject the null hypothesis at a 5% level, indicating that the vast majority of the portfolios seems to have time-varying market betas and intercept terms.
