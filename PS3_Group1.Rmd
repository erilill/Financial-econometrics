...
\alpha_{Period\;2} &=\alpha+\beta*Preriod\;2 \\
\alpha_{Period\;3} &=\alpha +\beta*Preriod\;3\\
\end{aligned}
$$
With $\beta$ denoting the corresponding coefficients, and the \emph{Period} variables are the dummy variables for the different periods. 

In order to include period-specific betas, we include interaction terms between the period dummies and the market excess:

$$
\beta(Preriod\;2*Z_{m}) \quad and \quad \beta(Preriod\;3*Z_m)
$$
Such that the CAPM model is:

$$
Z_{t}=\alpha_t+\beta_1*Z_{mt}+\beta_2*Preriod\;2+\beta_3*Preriod\;3+\beta_4(Preriod\;2*Z_{m}) + \beta_4(Preriod\;3*Z_m)+\epsilon_{t}
$$
...
This is a clear indication that the inclusion of period-specific effects increases the model's goodnes-of-fit.
