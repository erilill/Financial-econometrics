...
In this exercise you have to estimate and to test a multi-factor model with portfolios as factors. Is the data set portfolio_m
...
kable(variance_table, caption="The variation of the 100 portfolios that can be explained by
      the first 5 principal components. In percentages.")
```
...
kable(variance_table, caption="The variation of the 20 portfolios that can be explained by
      the first 5 principal components. In percentages.")
```
...
kable(descr_100, digits = 3, caption = "Descriptive statistics of the factor portfolios for the
...
kable(descr_20, digits = 3, caption = "Descriptive statistics of the factor portfolios for the
      20 size and book-to-market portfolios.")
```
...
kable(round(correlations_with_market,2), caption = "Correlation of each factor portfolio
      (of the 100 size-book-to-market) with the market portfolio:")

correlations_with_market_20 <- sapply(colnames(pc_scores_20), function(factor) {
  cor(pc_scores_20[,factor], portfolio_m[,3])
})

kable(round(correlations_with_market_20,2), caption = "Correlation of each factor portfolio
      (of the 20 size and book-to-market) with the market portfolio:")
```
...
#5 Principal components based on the 100 sbtm portfolios
pca_res_100 <- PCA(as.matrix(portfolio_m[,25:124]), 5)
pc_scores_100 <- pca_res_100$factors
...
We can see in table 8 how much of the time series variations of the portfolio returns which can be explained by the individual principal components. We see that for all portfolios except R1010, one factor/1 principal component explains over 50\% of the time series variation of the portfolio returns. For R1010 the 1 factor model only explains 22\% of the variation. For all portfolios except R1010, the 5 factor model explains between 68-86\%, depending on the portfolio, so quite high values. For the R1010 model, the 5 factor model only explains 37\%.

We can also see that when we include more than 1 factor, the $R^2$ increases for all portfolios, however including 2,3,4 factors more does not to much in terms of increasing $R^2$, so the goodness of fit does not increase that strongly when we include more than one factor. Except maybe for R1010 where the $R^2$ goes from 22\% to 37\% from the 1 factor model to the 5 factor model.

We can look at what is the optimal number of factors according tot the Bayes/Schwarz IC (BIC), i.e. which factor model has the lowest BIC value. In table 9 we can see that the BIC suggests that optimal number of factors are R11: 5 , R15: 5, R110: 5, R51: 4, R55: 4, R510: 5, R101: 5, R105: 4, and R1010: 5. We note that the difference is very small for the portfolios where it suggests 4 instead of 5 factors. So the optimal number of factors according to BIC for all the portfolios seem to be 4-5 factors, depending on the portfolio.
...
For the portfolios R51, R55, R510 and R1010, the difference in explanatory power of the two sets of principal components is really really small. For R11,R15, the PCA based on 100 Rx(x)y(y) portfolios has higher explanatory power, and for the R110, R101, R105, the PCA based on 20 MEx/MEBEx portfolios provide higher explanatory power.
...
colnames(R2_table_final) <- c("Portfolio", "1 Factor", "2 Factors", "3 Factors", "4 Factors",
                              "5 Factors", "1 Factor - Market risk premium")
...
In Table 11 we can see that the first principal component have more explanatory power than the market risk premium solely for all portfolios except R101, R105 and R1010. But when we increase the principal components, i.e. we have multi-factor models, wee see that for R105 and R1010 the multi factor models have more explanatory power over the market risk premium. Only for R101 the market risk premium has any explanatory power beyond the market risk premium. In table 12 we can look at the significance of the market risk premium, and we can see that the p-value is 0 for all portfolios. So we can conclude that the market risk premium is significant for all portfolios.
...
We can see that highest goodness of fit is achieved by the R55 portfolio, followed closely by the R110 portfolio. So there is a high goodness of fit for mid-size, mid-book-to-market, and large size, low book-to-market. The worst goodness of fit is for the R1010 portfolio, i.e. the goodness of fit is really low for large size high book-to-market.

Perhaps we can view these findings as if the portfolio is only of large size and high book to market, we barely see any explanatory power using multi-factor models beyond the market risk premium, which means that the market risk premium basically explains the returns of that portfolio. But for other portfolios, such as the highest ones, mid-mid and large-low, we see that we can not properly explain the (excess returns) with just the market risk premium, and there a multi-factor model works really well.
...
$$
\hat{\alpha}=\bar{Z}-\hat{B}\hat{\mu}_K, \\
\text{where }
\bar{Z}=\frac{1}{T} \sum_{t=1}^{T}Z_t, \text{ }\hat{\mu}_k=\frac{1}{T}\sum_{t=1}^{T}Z_{Kt},\\
\hat{B}= \left[\sum_{t=1}^{T}(Z_t-\bar{Z})(Z_{Kt}-\hat{\mu}_k)' \right]\left[\sum_{t=1}^{T} (Z_t-\hat{\mu}_K)(Z_{Kt}-\hat{\mu}_k)' \right]
$$
The covariance matrix of the error terms are estimated by maximum likelihood: 
$$
\hat{\Sigma}=\frac{1}{T} \sum_{t=1}^{T} (Z_t-\hat{\alpha}-\hat{B}Z_{Kt})(Z_t-\hat{\alpha}-\hat{B}Z_{Kt})'
$$
...
sur <- function(Z, Z_k){
  ...
  return(list(mu_k = mu_k, B_hat = B_hat, alpha_hat = alpha_hat, Sigma_hat = Sigma_hat,
              residuals = residuals, BIC=BIC))
}
```
...
Test for exact factor pricing, i.e. do the F-test with null $\alpha=0$. Note that we estimate the model as a system in GRS_F() as we call the sur() estimation function in there.
...
We can see that we reject the null that $\alpha=0$. That means that we do not have evidence that there is exact factor pricing in all equations, i.e. there is at least one equation where $\alpha \neq 0$.

Now we do the LR test for the joint significance of the factors, i.e. the do LR test with the restricted model where $B=0$
...
We can see that we reject the null model, i.e. the restricted model quite strongly, in favor of the unrestricted model. So we have strong evidence of joint significance of the factors.
...
Now we estimate the 1 factor market risk premium model
...
We can see that we reject the null model, i.e. the restricted model quite strongly, in favor of the unrestricted model. So we have strong evidence of joint significance of the market risk premium in all equations.

Now to compare the goodness-of-fit of alternative factor specifications using information criteria (BIC). We compare the 1 factor market risk premium model with the 5 factor model with the principal components based on the 20 size and book-to-market portfolios.

```{r}
sur_ur_5f$BIC # 5 factor model with principal components
sur_ur_1f$BIC # 1 factor model with market risk premium
```
We can see that the 5 factor model with the principal components has a better goodness of fit than the 1 factor model with the market risk premium, for the complete system, since it has a lower BIC value.
