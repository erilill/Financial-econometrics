...
The BIC suggests that optimal number of factors are R11: 5 , R15: 5, R110: 5, R51: 4, R55: 4, R510: 5, R101: 5, R105: 4, and R1010: 5. Please notice that the difference is very small for the portfolios where it suggests 4 instead of 5 concluding that it doesn't influence the effectiveness of the model greatly but considering that we always prefer less complex models, we should go for the 4-facotr models when it is suggested. 
...
colnames(sur_data)[101:105] <- c("PC1", "PC2", "PC3", "PC4", "PC5")

sur_model <- systemfit(equations, method = "SUR", data = sur_data)
```
...
mu_k <- matrix(colMeans(sur_data[, c("PC1", "PC2", "PC3", "PC4", "PC5")]))

Z_kt <- as.matrix(sur_data[, c("PC1", "PC2", "PC3", "PC4", "PC5")])
...
fit <- lm(mZ[,1]~mZk)
fit$coefficients
head(c(alpha_hat,B_hat))
# Step 3: Estimate alpha
alpha_hat <- Z_bar - B_hat %*% mu_k

# Step 4: Estimate Sigma
residuals <- mZ - (matrix(alpha_hat, nrow = T, ncol = ncol(mZ), byrow = TRUE) + mZk %*% t(B_hat))
