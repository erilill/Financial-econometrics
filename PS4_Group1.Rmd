...
# Loop through the portfolio and factors
for (i in 1:length(portfolio)){
  for (j in 1:length(factors)){
    # Fit the model
    model <- lm(mZ[, portfolio[i]] ~ pc_scores_excess[, 1:j])
    # Extract and store the R-squared value
    mR2[i, j] <- summary(model)$r.squared
  }
}

R2_table <- cbind(portfolio,round(mR2,2))
colnames(R2_table) <- c("Portfolio", "1 Factor", "2 Factors", "3 Factors", "4 Factors", "5 Factors")
kable(R2_table, caption = "R-squared values for different factor models")
